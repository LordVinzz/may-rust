...
class CompTraceur(ABC):
    ...
    class _Parts:
        ...
        def traceur(self) -> Traceur:
            return self._owner._get_part_traceur()
    ...
    def __init__(self):
        ...
        self._part_traceur_cache = _UNSET
    ...
    @abstractmethod
    def make_traceur(self) -> Traceur:
        raise NotImplementedError('implement `make_traceur` in a concrete component')

    def _get_part_traceur(self) -> Traceur:
        if self._part_traceur_cache is _UNSET:
            part = self.make_traceur()
            self._part_traceur_cache = part
            part._bind_starter(self.parts().simple().starter())
        return self._part_traceur_cache
    ...
    def _get_part_client(self) -> Client:
        if self._part_client_cache is _UNSET:
            ...
            part._bind_demarreur(self.parts().traceur().demarreur())
